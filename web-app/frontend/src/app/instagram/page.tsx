...
export default function InstagramPage() {
  ...
  const filteredShorts = hidePublished 
    ? shorts.filter(s => !s.instagram_published) 
    : shorts;

  const fetchShorts = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/instagram/shorts`);
      if (res.ok) {
  ...
  // Optimiser le texte pour Instagram (limite 2200 caractères, hashtags à la fin)
  const getInstagramCaption = (short: ShortItem) => {
    const hashtags = short.tags?.slice(0, 10).map(t => `#${t.replace(/\s+/g, '')}`).join(' ') || '';
    const instagramHashtags = '#reels #reelsinstagram #viral #explore';
    const desc = short.description?.substring(0, 200) || '';
    return `${short.title}\n\n${desc}\n\n.\n.\n.\n\n${hashtags} ${instagramHashtags}`;
  };

  const copyAllForInstagram = (short: ShortItem) => {
    const caption = getInstagramCaption(short);
    copyToClipboard(caption, `all-${short.project_id}-${short.short_index}`);
  };

  const togglePublished = async (short: ShortItem) => {
    try {
      const res = await fetch(`${API_URL}/api/instagram/shorts/mark-published`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          folder_name: short.folder_name,
          short_index: short.short_index,
          published: !short.instagram_published
        })
      });
      if (res.ok) {
        setShorts(prev => prev.map(s => 
          s.folder_name === short.folder_name && s.short_index === short.short_index
            ? { ...s, instagram_published: !s.instagram_published, instagram_published_at: new Date().toISOString() }
            : s
        ));
      }
    } catch (err) {
    ...
    <div className="min-h-screen p-8">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-zinc-500 text-sm">Préparez vos shorts pour Instagram</p>
            </div>
          </div>
        </div>
        <button
        ...
        <a
          ...
          className="px-4 py-2 bg-gradient-to-r from-purple-600 via-pink-500 to-orange-400 hover:opacity-90 text-white font-medium rounded-lg transition flex items-center gap-2"
        >
      ...
      <div className="card p-4 mb-6 border-pink-500/20">
        ...
        <ol className="text-sm text-zinc-400 space-y-1 list-decimal list-inside">
          <li>Téléchargez le short sur votre téléphone ou PC</li>
          <li>Cliquez sur <strong className="text-white">Copier pour Instagram</strong> pour copier la description + hashtags</li>
          <li>Ouvrez <strong className="text-white">Instagram</strong> → Créer → Reel</li>
          <li>Importez la vidéo, collez la description et publiez !</li>
        </ol>
      </div>
      ...
      {shorts.length > 0 && (
        <div className="flex items-center justify-between mb-4">
          ...
          >
            {hidePublished ? (
              <>
                ...
                Publiés cachés ({shorts.filter(s => s.instagram_published).length})
              </>
            ) : (
              'Afficher tout'
            )}
          ...
          <span className="text-sm text-zinc-500">
            {filteredShorts.length} reel{filteredShorts.length > 1 ? 's' : ''} affiché{filteredShorts.length > 1 ? 's' : ''}
          </span>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          <Instagram className="w-16 h-16 text-zinc-600 mx-auto mb-4" />
          <p className="text-zinc-400 text-lg mb-2">Aucun reel disponible</p>
          <p className="text-zinc-500 text-sm">
            Les reels seront générés automatiquement après le traitement de vos vidéos.
          </p>
            ...
            <div key={`${short.project_id}-${short.short_index}`} className="card overflow-hidden group">
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-2 text-xs text-zinc-500">
                  ...
                  <span>•</span>
                  <span>Reel {short.short_index + 1}</span>
                </div>

                {/* Title */}
                <h3 className="font-medium text-white line-clamp-2 text-sm">
                  {short.title || `Reel ${short.short_index + 1}`}
                </h3>
                ...
                {short.tags && short.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {short.tags.slice(0, 4).map((tag, i) => (
                      <span key={i} className="text-xs bg-purple-500/10 text-purple-400 px-2 py-0.5 rounded-full">
                        #{tag}
                      </span>
                    ))}
                    {short.tags.length > 4 && (
                ...
                <div className="space-y-2 pt-2">
                  <div className="flex gap-2">
                    <a
                      ...
                      download={`instagram_${short.folder_name}_reel${short.short_index + 1}.mp4`}
                    ...
                    <button
                      onClick={() => copyAllForInstagram(short)}
                      className="flex-1 py-2 bg-gradient-to-r from-purple-600 via-pink-500 to-orange-400 hover:opacity-90 text-white text-sm font-medium rounded-lg transition flex items-center justify-center gap-2"
                    >
                  ...
                  >
                    <Check className="w-4 h-4" />
                    {short.instagram_published ? 'Publié sur Instagram ✓' : 'Marquer comme publié'}
                  </button>
                </div>
      ...
      {shorts.length > 0 && (
        <div className="mt-8 card p-4 flex items-center justify-between">
          <div className="text-sm text-zinc-400 flex items-center gap-4">
            <span>
              <span className="text-white font-semibold">{shorts.length}</span> reels disponibles
            </span>
            <span className="text-green-400">
              <Check className="w-4 h-4 inline mr-1" />
              {shorts.filter(s => s.instagram_published).length} publiés
            </span>
            <span className="text-purple-400">
              {shorts.filter(s => !s.instagram_published).length} à publier
            </span>
          </div>
          <div className="flex items-center gap-2 text-xs text-zinc-500">
            <Clock className="w-4 h-4" />
            Format : 9:16 vertical, 15-90 sec
          </div>
        </div>
      )}
    </div>
